...
![Diagram showing how the task chaining workflow pattern works](https://docs.dapr.io/images/workflow-overview/workflows-chaining.png)
...
![Diagram showing how the fan-out/fan-in workflow pattern works](https://docs.dapr.io/images/workflow-overview/workflows-fanin-fanout.png)

In addition to the challenges mentioned in [the previous pattern](https://docs.dapr.io/developing-applications/building-blocks/workflow/workflow-patterns/#task-chaining), there are several important questions to consider when implementing the fan-out/fan-in pattern manually:
...
![Diagram showing how the async request response pattern works](https://docs.dapr.io/images/workflow-overview/workflow-async-request-response.png)
...
![Diagram showing how the monitor pattern works](https://docs.dapr.io/images/workflow-overview/workflow-monitor-pattern.png)
...
Dapr Workflow supports this pattern natively by allowing you to implement *eternal workflows*. Rather than writing infinite while-loops ([which is an anti-pattern](https://docs.dapr.io/developing-applications/building-blocks/workflow/workflow-features-concepts/#infinite-loops-and-eternal-workflows)), Dapr Workflow exposes a *continue-as-new* API that workflow authors can use to restart a workflow function from the beginning with a new input.
...
In some cases, a workflow may need to pause and wait for an external system to perform some action. For example, a workflow may need to pause and wait for a payment to be received. In this case, a payment system might publish an event to a pub/sub topic on receipt of a payment, and a listener on that topic can raise an event to the workflow using the [raise event workflow API](https://docs.dapr.io/developing-applications/building-blocks/workflow/howto-manage-workflow/#raise-an-event).

Another very common scenario is when a workflow needs to pause and wait for a human, for example when approving a purchase order. Dapr Workflow supports this event pattern via the [external events](https://docs.dapr.io/developing-applications/building-blocks/workflow/workflow-features-concepts/#external-events) feature.
...
![Diagram showing how the external system interaction pattern works with a human involved](https://docs.dapr.io/images/workflow-overview/workflow-human-interaction-pattern.png)
...
The code that delivers the event to resume the workflow execution is external to the workflow. Workflow events can be delivered to a waiting workflow instance using the [raise event](https://docs.dapr.io/developing-applications/building-blocks/workflow/howto-manage-workflow/#raise-an-event) workflow management API, as shown in the following example:
...
![Diagram showing how the compensation pattern.](https://docs.dapr.io/images/workflow-overview/workflows-compensation.png)
